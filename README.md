...
* `deltaXAndYEvents`: Number of events that contain both non-zero delta X and non-zero delta Y at the same time.
...
3. Any events with both non-zero deltaX and deltaY → `'trackpad'`
4. Any events with non-zero deltaX → `'trackpad'`
